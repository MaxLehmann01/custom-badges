...
export default class Database {
    ...
    public async start(): Promise<void> {
        if (!Database.config) {
            throw new Error('Database configuration not set. Call Postgres.setConfig() first.');
        }
        ...
        this.pool.on('connect', () => {
            this.logger.info(`Connected to Database at ${user}@${host}:${port}/${database}`);
        });
    ...
    public async migrate(): Promise<void> {
        const migrationsDirectory = Database.config.migrationsDirectory;

        if (!fs.existsSync(migrationsDirectory)) {
            this.logger.error('Migrations directory does not exist', { migrationsDirectory });
        ...
        if (scripts.length === 0) {
            this.logger.warn('Migrations directory is empty', { migrationsDirectory });
        }
        ...
        try {
            ...
            for (const script of scripts) {
                const { rows } = await client.query(`SELECT 1 FROM ${this.schema}._migrations WHERE filename = $1`, [
                    script,
                ]);

                if (rows.length > 0) {
                    this.logger.debug('Migration already applied, skipping', { script });
                    continue;
                }

                const fullFilePath = path.join(migrationsDirectory, script);
                ...
                try {
                    ...
                    await client.query(`INSERT INTO ${this.schema}._migrations (filename) VALUES ($1)`, [script]);
                    await client.query('COMMIT');

                    this.logger.info('Migration applied successfully', { script });
                } catch (e) {
                    await client.query('ROLLBACK');
                    this.logger.error('Error applying migration, rolled back', { script });
    ...
    public async query<T extends QueryResultRow>(
        query: string,
        params: Array<string | number | boolean | Date | null | string[] | Buffer> = []
    ...
    public async insert<T extends string | number | boolean | Date | Buffer | string[]>(
        table: string,
        data: Record<string, string | number | boolean | Date | null | string[] | Buffer>,
